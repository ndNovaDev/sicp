...
; (define (streamLimit s t)
;   (if (< (abs (- (streamCar s) (streamCar (streamCdr s)))) t)
;       (streamCar (streamCdr s))
;       (streamLimit (streamCdr s) t)))

(define (streamLimit s t)
  (let ((a (streamCar s))
        (b (streamCar (streamCdr s))))
       (if (< (abs (- a b))
              t)
           b
           (streamLimit (streamCdr s) t))))
