...
(define (apply-generic op . ori-args)
  (define (transform type)
    (define (iter ret rest)
      (if (null? rest)
          ret
          (let ((transformer (get-coercion (type-tag (car rest)) type)))
               (if transformer
                   (iter (append ret (transformer (car rest)))
                         (cdr rest))
                   #f))))
    (iter '() ori-args))
  (define (try-next rest)
    (if (null? rest)
        (error "mei la" "meila")
        (let ((type (type-tag (car rest))))
          (let ((next (transform type)))
              (if next
                  (iter rest next)
                  (try-next (cdr rest)))))))
